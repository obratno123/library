...
Рекомендации реализованы двумя способами:

1. На основе покупок пользователей:
   - анализируются пользователи, купившие текущую книгу;
   - подбираются книги, которые они также покупали.

2. На основе содержимого книги (ML):
   - используется модель `sentence-transformers/all-MiniLM-L6-v2`;
   - модель устанавливается при первичном запуске
   - учитываются название, авторы, жанры и описание;
   - рассчитывается косинусное сходство между книгами;
   - выводятся наиболее похожие книги.

Если данных о покупках недостаточно, используется второй подход.
...
- система статусов доставки заказа;
- статус рассчитывается на основе времени оплаты заказа.
...
- `/cart/order/<order_id>/` — детальная страница заказа со статусом доставки, оплатой, составом и суммой
- `/cart/order/<order_id>/success/` — страница успешного оформления заказа
...
Для работы рекомендаций по содержимому необходимо выполнить:
